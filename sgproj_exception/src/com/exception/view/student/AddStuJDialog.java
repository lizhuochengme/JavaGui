...
public class AddStuJDialog extends JDialog {
	...
	public AddStuJDialog(JFrame frame,boolean flag) {
		...
		JLabel lblNewLabel_1_1 = new JLabel("部  门");
...
//		this.headPic.setBounds(300, 20, 100, 130);
		...
		JButton btnNewButton = new JButton("职工入住");
...
//		String photo = this.headPic.getFilePath();
		...
		if(!StrUtil.notEmpty(stuName)||!StrUtil.notEmpty(phone)||date == null||classBean == null||roomBean == null) {
...
//        }
		...
		//boolean addStudentBool = stuService.addStudent(new StudentBean(stuName, gender, phone, date, roomBean, classBean));
        boolean addStudentBool = stuService.addStudent(new StudentBean(stuName, gender, "", phone, date, roomBean, classBean));
		if(addStudentBool) {
			JOptionPane.showMessageDialog(this, "职工入住成功");
			initialJDialog();
			return;
		}else {
